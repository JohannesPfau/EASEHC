...
public class Meshinator : MonoBehaviour
{
    ...
    public GameObject caronteFracturedObject;
	...
	{
		m_CollisionCount++;
        if (m_ImpactType == ImpactTypes.Cuttable)
        {
            if (collision.gameObject.GetComponent<isCuttingObject>() != null)
            {
                Debug.Log("Collision with cutting object:" + collision.gameObject.name);
                // Find the impact point
                foreach (ContactPoint contact in collision.contacts)
                {
                    if (contact.otherCollider == collision.collider)
                    {
                        Debug.Log("impact point: " + contact.point + " force: " + collision.relativeVelocity);
                        Impact(contact.point, new Vector3(100,100,100), m_ImpactShape, m_ImpactType);
                        break;
                    }
                }
            }
    ...
    {
        caronteFracturedObject.transform.position = transform.position;
        caronteFracturedObject.transform.rotation = transform.rotation;
        caronteFracturedObject.SetActive(true);
        AudioManager.playAudio("Break", caronteFracturedObject);
    ...
    public void Impact(Vector3 point, Vector3 force, ImpactShapes impactShape, ImpactTypes impactType)
	{
        ...
        //if (impactType == ImpactTypes.Cuttable)
        //    impactFactor = 100;
        if (impactFactor <= 0)
            return;

        // Custom:
        if (impactType == ImpactTypes.CaronteFracture)
        {
            destroyMe();
        }
		...
		ThreadManager.RunAsync(()=>
		{
			...
			ThreadManager.QueueOnMainThread(()=>
			{
				...
				if (impactType == ImpactTypes.Fracture || impactType == ImpactTypes.Cuttable)
				{
